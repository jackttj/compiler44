...
ir::Value ir::Executor::find_src_operand(Operand op) {
    ...
    if (iter == cur_ctx->mem.end()) {
        ...
        assert(iter != global_vars.end() && "can not find the arguement in current conxtext or global variables " + op.name);
    } 
    retval = iter->second;
...
#ifdef DEBUG_EXEC_DETAIL
    std::cout << "this is the  draw of inst\n";
    ...
    std::cout << "\tin goto: pc = " << cur_ctx->pc << std::endl;
            ...
            case Operator::call: {
                ...
                if (exec_lib_function(callinst, &libfunc_retval)) {
                    std::cout << " yes I execute the lib fun\n";
